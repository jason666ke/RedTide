...
def get_column_statistics(dataframe):
    """
        该函数计算并输出数据框中每一列的基本统计信息，包括数据类型、非空值数量、缺失值数量、唯一值数量、最小值和最大值。
    """
...
def get_time_intervals(df):
    """
        计算相邻时间戳之间的时间差，包括平均时间间隔、中位数、最小值和最大值。
    """
...
def get_correlation(df, target):
    """
        该函数用于计算数据框中数值型列之间的相关性，并输出目标列（target）与其他列之间的相关性
    """
...
def std_err(df):
    """
        该函数用于计算 OT 列的均值和标准差。标准误差是衡量数据波动程度的重要指标，常用于表示数据的离散度。
    """
    ot_mean = df['OT'].mean()
    ot_std = df['OT'].std()
