...
output_dir = '/root/lhq/data/data_grouped_cls2'
red_tide_path = '/root/lhq/data/data_raw/赤潮整理.xlsx'

cols_to_keep = ['站点名称', '监测时间', '风速', '风向', '气温', '相对湿度', '气压', '雨量', '水温', '电导率', '盐度', 'ph', '氧化还原电位', '溶解氧', '浊度', '叶绿素a']
...
def map_red_tide_info(row):
    ...
    if key in red_tide_dict:
        return pd.Series([red_tide_dict[key], 1])
    else:
