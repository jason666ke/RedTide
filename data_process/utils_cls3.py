...
#     features_to_remove = ['Unnamed: 0', '雨量', '浊度', '氧化还原电位', '风速', '风向']

#     feature_remove = '雨量浊度氧化还原电位风速风向'
...
if __name__ == "__main__":
    ...
    features_to_remove = ['雨量', '浊度', '氧化还原电位', '风速', '风向']
    ...
    # 保存ts文件
    data_dict = {
        'train': (df, paths['train_ts']),
        'test': (df, paths['test_ts'])
    }
    for _, (df, path) in data_dict.items():
        sequence, label = to_ts(df)
        save_ts(sequence, label, path, 'AllRedTide')
